...
import com.L0v4iy.deezer.domain.*;
import com.L0v4iy.deezer.domain.internal.*;
import com.L0v4iy.deezer.domain.internal.search.*;
...
public class DeezerClientTest {
    ...
    public void getTrack() {
        // deprecated test in 2021.01.10 not worked
        String trackId = "3135556";
        Track track = deezerClient.get(new TrackId(Long.parseLong(trackId)));
        assertEquals(Long.parseLong(trackId), track.getId().longValue());
        assertEquals("Harder Better Faster Stronger", track.getTitle());
        assertNotNull(track.getAvailable_countries());
    }

    @Test
    public void getModernTrack() {
        // correct test on 2021.01.10
        String trackId = "84325325";
        Track track = deezerClient.get(new TrackId(Long.parseLong(trackId)));
        assertEquals(Long.parseLong(trackId), track.getId().longValue());
        assertEquals("Invader", track.getTitle());
        assertNotNull(track.getAvailable_countries());
    }
